...
@interface FXHTTPNetworkSession() <NSURLSessionDataDelegate>
...
@property (nonatomic, strong) NSMutableDictionary<NSURLSessionTask *, FXHTTPNetworkTaskData *> * sessions;
@property (nonatomic, strong) dispatch_queue_t sessionsQueue;
@end

@implementation FXHTTPNetworkSession
...
- (instancetype)init {
    if (self = [super init]) {
        // Shared `NSURLSession` to be used for all `FXHTTPNetworkTask` objects. All tasks should use this single
        ...
        _sessionsQueue = dispatch_queue_create("com.fxsdk.FXHTTPnetworksession.queue", DISPATCH_QUEUE_CONCURRENT);
    }
...
- (void)setSessionData:(FXHTTPNetworkTaskData *)data forTask:(NSURLSessionTask *)task {
 ...
 */
- (FXHTTPNetworkTaskData *)sessionDataForTask:(NSURLSessionTask *)task {
    __block FXHTTPNetworkTaskData * data = nil;
...
/**
 Appends additional data to the @c responseData field of @c FXHTTPNetworkTaskData in
...
- (void)appendData:(NSData *)data toSessionDataForTask:(NSURLSessionTask *)task {
    ...
    dispatch_barrier_sync(self.sessionsQueue, ^{
        ...
        FXHTTPNetworkTaskData * taskData = self.sessions[task];
        if (taskData == nil) {
            return;
        }
...
+ (NSURLSessionTask *)taskWithHttpRequest:(NSURLRequest *)request
    ...
    // Networking task
    NSURLSessionDataTask * task = [FXHTTPNetworkSession.sharedInstance.sharedSession dataTaskWithRequest:request];

    // Initialize the task data
    FXHTTPNetworkTaskData * taskData = [[FXHTTPNetworkTaskData alloc] initWithResponseHandler:responseHandler errorHandler:errorHandler shouldRedirectWithNewRequest:shouldRedirectWithNewRequest];

    // Update the sessions.
    [FXHTTPNetworkSession.sharedInstance setSessionData:taskData forTask:task];
...
+ (NSURLSessionTask *)startTaskWithHttpRequest:(NSURLRequest *)request {
    return [FXHTTPNetworkSession startTaskWithHttpRequest:request responseHandler:nil errorHandler:nil shouldRedirectWithNewRequest:nil];
}

+ (NSURLSessionTask *)startTaskWithHttpRequest:(NSURLRequest *)request
                               responseHandler:(void (^ _Nullable)(NSData * data, NSHTTPURLResponse * response))responseHandler
                                  errorHandler:(void (^ _Nullable)(NSError * error))errorHandler {
    return [FXHTTPNetworkSession startTaskWithHttpRequest:request responseHandler:responseHandler errorHandler:errorHandler shouldRedirectWithNewRequest:nil];
}
...
+ (NSURLSessionTask *)startTaskWithHttpRequest:(NSURLRequest *)request
    ...
    NSURLSessionTask * task = [FXHTTPNetworkSession taskWithHttpRequest:request responseHandler:^(NSData * _Nonnull data, NSHTTPURLResponse * _Nonnull response) {
...
willPerformHTTPRedirection:(NSHTTPURLResponse *)response
 ...
 completionHandler:(void (^)(NSURLRequest * _Nullable))completionHandler {
    ...
    FXHTTPNetworkTaskData * taskData = [self sessionDataForTask:task];
    if (taskData == nil) {
...
//        MPLogEvent([MPLogEvent error:error message:nil]);
    ...
    if (httpResponse == nil) {
        NSString *description = @"Network response is not of type NSHTTPURLResponse";
        NSError * notHttpResponseError = [NSError errorWithDomain:@"com.fxsdk.com" code:2 userInfo:@{ NSLocalizedDescriptionKey: description }];
//        MPLogEvent([MPLogEvent error:notHttpResponseError message:nil]);
    ...
    if (httpResponse.statusCode >= 400) {
        NSString *description = [NSString stringWithFormat:@"Network httpResponse.statusCode is %ld",(long)httpResponse.statusCode];
        NSError * not200ResponseError = [NSError errorWithDomain:@"com.fxsdk.com" code:2 userInfo:@{ NSLocalizedDescriptionKey: description}];
...
//        MPLogEvent([MPLogEvent error:not200ResponseError message:nil]);
    ...
    if (taskData.responseData == nil) {
        NSString *description = @"No data found in the NSHTTPURLResponse";
        NSError * noDataError = [NSError errorWithDomain:@"com.fxsdk.com" code:2 userInfo:@{ NSLocalizedDescriptionKey: description}];
