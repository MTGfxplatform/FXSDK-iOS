...
@interface FXSDKFMDatabase ()

- (FXSDKFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args;
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args;

@end

@implementation FXSDKFMDatabase
...
#pragma mark FXSDKFMDatabase instantiation and deallocation
...
- (void)dealloc {
    [self close];
    FXSDKFMDBRelease(_openResultSets);
    FXSDKFMDBRelease(_cachedStatements);
    FXSDKFMDBRelease(_dateFormat);
    FXSDKFMDBRelease(_databasePath);
    FXSDKFMDBRelease(_openFunctions);
...
// /* need to make sure to do X with FXSDKFMDB version 2.4 or later */
// if ([FXSDKFMDatabase FXSDKFMDBVersion] >= 0x0240) { … }

+ (SInt32)FXSDKFMDBVersion {
    
    // we go through these hoops so that we only have to change the version number in a single spot.
    static dispatch_once_t once;
    static SInt32 FXSDKFMDBVersionVal = 0;
    
    dispatch_once(&once, ^{
        NSString *prodVersion = [self FXSDKFMDBUserVersion];
        ...
        char *e = nil;
        FXSDKFMDBVersionVal = (int) strtoul([junk UTF8String], &e, 16);
        
    });
    

    return FXSDKFMDBVersionVal;
}
...
static int FXSDKFMDBDatabaseBusyHandler(void *f, int count) {
    FXSDKFMDatabase *self = (__bridge FXSDKFMDatabase*)f;
...
- (void)setMaxBusyRetryTimeInterval:(NSTimeInterval)timeout {
    ...
    if (timeout > 0) {
        sqlite3_busy_handler(_db, &FXSDKFMDBDatabaseBusyHandler, (__bridge void *)(self));
    }
    else {
...
// but for folks who don't bother noticing that the interface to FXSDKFMDatabase changed,
// we'll still implement the method so they don't get suprise crashes
- (int)busyRetryTimeout {
    DLog(@"%s:%d", __FUNCTION__, __LINE__);
    DLog(@"FXSDKFMDB: busyRetryTimeout no longer works, please use maxBusyRetryTimeInterval");
    return -1;
}
...
#pragma unused(i)
    DLog(@"%s:%d", __FUNCTION__, __LINE__);
    DLog(@"FXSDKFMDB: setBusyRetryTimeout does nothing, please use setMaxBusyRetryTimeInterval:");
}
...
- (void)closeOpenResultSets {
    ...
    NSSet *openSetCopy = FXSDKFMDBReturnAutoreleased([_openResultSets copy]);
...
- (void)resultSetDidClose:(FXSDKFMResultSet *)resultSet {
...
- (FXSDKFMStatement*)cachedStatementForQuery:(NSString*)query {
    
    NSMutableSet* statements = [_cachedStatements objectForKey:query];
    
    return [[statements objectsPassingTest:^BOOL(FXSDKFMStatement* statement, BOOL *stop) {
...
- (void)setCachedStatement:(FXSDKFMStatement*)statement forQuery:(NSString*)query {
...
+ (NSDateFormatter *)storeableDateFormat:(NSString *)format {
    ...
    NSDateFormatter *result = FXSDKFMDBReturnAutoreleased([[NSDateFormatter alloc] init]);
    result.dateFormat = format;
    result.timeZone = [NSTimeZone timeZoneForSecondsFromGMT:0];
    result.locale = FXSDKFMDBReturnAutoreleased([[NSLocale alloc] initWithLocaleIdentifier:@"en_US"]);
    return result;
}
...
- (void)setDateFormat:(NSDateFormatter *)format {
    FXSDKFMDBAutorelease(_dateFormat);
    _dateFormat = FXSDKFMDBReturnRetained(format);
}
...
- (BOOL)goodConnection {
    ...
    FXSDKFMResultSet *rs = [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (void)warnInUse {
    DLog(@"The FXSDKFMDatabase %@ is currently in use.", self);
    
#ifndef NS_BLOCK_ASSERTIONS
    if (_crashOnErrors) {
        NSAssert(false, @"The FXSDKFMDatabase %@ is currently in use.", self);
        abort();
    }
#endif
}
...
- (BOOL)databaseExists {
    ...
    if (!_db) {
        ...
        DLog(@"The FXSDKFMDatabase %@ is not open.", self);
        
    #ifndef NS_BLOCK_ASSERTIONS
        if (_crashOnErrors) {
            NSAssert(false, @"The FXSDKFMDatabase %@ is not open.", self);
            abort();
        }
    #endif
...
- (NSError*)errorWithMessage:(NSString*)message {
    ...
    return [NSError errorWithDomain:@"FXSDKFMDatabase" code:sqlite3_errcode(_db) userInfo:errorMessage];    
}
...
- (FXSDKFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    FXSDKFMStatement *statement  = 0x00;
    FXSDKFMResultSet *rs         = 0x00;
    ...
    if (dictionaryArgs) {
        ...
        for (NSString *dictionaryKey in [dictionaryArgs allKeys]) {
            ...
            FXSDKFMDBRelease(parameterName);
    ...
    FXSDKFMDBRetain(statement); // to balance the release below
    
    if (!statement) {
        statement = [[FXSDKFMStatement alloc] init];
    ...
    rs = [FXSDKFMResultSet resultSetWithStatement:statement usingParentDatabase:self];
    [rs setQuery:sql];
...
- (FXSDKFMResultSet *)executeQueryWithFormat:(NSString*)format, ... {
    va_list args;
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    int rc                   = 0x00;
    sqlite3_stmt *pStmt      = 0x00;
    FXSDKFMStatement *cachedStmt  = 0x00;
    ...
    if (_shouldCacheStatements && !cachedStmt) {
        cachedStmt = [[FXSDKFMStatement alloc] init];
        
        [cachedStmt setStatement:pStmt];
        
        [self setCachedStatement:cachedStmt forQuery:sql];
        
        FXSDKFMDBRelease(cachedStmt);
    }
...
int FXSDKFMDBExecuteBulkSQLCallback(void *theBlockAsVoid, int columns, char **values, char **names); // shhh clang.
int FXSDKFMDBExecuteBulkSQLCallback(void *theBlockAsVoid, int columns, char **values, char **names) {
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(FXSDKFMDBExecuteStatementsCallbackBlock)block {
    ...
    rc = sqlite3_exec([self sqliteHandle], [sql UTF8String], block ? FXSDKFMDBExecuteBulkSQLCallback : nil, (__bridge void *)(block), &errmsg);
...
static NSString *FXSDKFMDBEscapeSavePointName(NSString *savepointName) {
...
- (BOOL)startSavePointWithName:(NSString*)name error:(NSError**)outErr {
    ...
    NSString *sql = [NSString stringWithFormat:@"savepoint '%@';", FXSDKFMDBEscapeSavePointName(name)];
...
- (BOOL)releaseSavePointWithName:(NSString*)name error:(NSError**)outErr {
    ...
    NSString *sql = [NSString stringWithFormat:@"release savepoint '%@';", FXSDKFMDBEscapeSavePointName(name)];
...
- (BOOL)rollbackToSavePointWithName:(NSString*)name error:(NSError**)outErr {
    ...
    NSString *sql = [NSString stringWithFormat:@"rollback transaction to savepoint '%@';", FXSDKFMDBEscapeSavePointName(name)];
    BOOL worked = [self executeUpdate:sql];
...
void FXSDKFMDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv); // -Wmissing-prototypes
void FXSDKFMDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv) {
#if ! __has_feature(objc_arc)
...
- (void)makeFunctionNamed:(NSString*)name maximumArguments:(int)count withBlock:(void (^)(sqlite3_context *context, int argc, sqlite3_value **argv))block {
    ...
    id b = FXSDKFMDBReturnAutoreleased([block copy]);
    
    [_openFunctions addObject:b];
    
    /* I tried adding custom functions to release the block when the connection is destroyed- but they seemed to never be called, so we use _openFunctions to store the values instead. */
#if ! __has_feature(objc_arc)
    sqlite3_create_function([self sqliteHandle], [name UTF8String], count, SQLITE_UTF8, (void*)b, &FXSDKFMDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#else
    sqlite3_create_function([self sqliteHandle], [name UTF8String], count, SQLITE_UTF8, (__bridge void*)b, &FXSDKFMDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#endif
}
...
@implementation FXSDKFMStatement
