...
+ (instancetype)resultSetWithStatement:(FXSDKFMStatement *)statement usingParentDatabase:(FXSDKFMDatabase*)aDB {
    ...
    return FXSDKFMDBReturnAutoreleased(rs);
}
...
- (void)dealloc {
    ...
    FXSDKFMDBRelease(_columnNameToIndexMap);
...
- (NSDictionary*)resultDict {
    ...
    if (num_cols > 0) {
        ...
        return FXSDKFMDBReturnAutoreleased([dict copy]);
    }
    else {
    ...
    if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
        ...
        if (outErr) {
            if (_parentDB) {
                ...
                *outErr = [NSError errorWithDomain:@"FXSDKFMDatabase" code:SQLITE_MISUSE userInfo:errorMessage];
            }
