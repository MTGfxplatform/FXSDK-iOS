...
#import "FXSDKFMDatabasePool.h"
    ...
    #define FXSDKFMDBAutorelease(__v) ([__v autorelease]);
    #define FXSDKFMDBReturnAutoreleased FXSDKFMDBAutorelease

    #define FXSDKFMDBRetain(__v) ([__v retain]);
    #define FXSDKFMDBReturnRetained FXSDKFMDBRetain

    #define FXSDKFMDBRelease(__v) ([__v release]);

    #define FXSDKFMDBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
    // -fobjc-arc
    #define FXSDKFMDBAutorelease(__v)
    #define FXSDKFMDBReturnAutoreleased(__v) (__v)

    #define FXSDKFMDBRetain(__v)
    #define FXSDKFMDBReturnRetained(__v) (__v)

    #define FXSDKFMDBRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details. 
    #if OS_OBJECT_USE_OBJC
        #define FXSDKFMDBDispatchQueueRelease(__v)
    #else
        #define FXSDKFMDBDispatchQueueRelease(__v) (dispatch_release(__v));
    #endif
#endif
...
typedef int(^FXSDKFMDBExecuteStatementsCallbackBlock)(NSDictionary *resultsDictionary);


/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 
 ### Usage
 The three main classes in FXSDKFMDB are:

 - `FXSDKFMDatabase` - Represents a single SQLite database.  Used for executing SQL statements.
 - `<FXSDKFMResultSet>` - Represents the results of executing a query on an `FXSDKFMDatabase`.
 - `<FXSDKFMDatabaseQueue>` - If you want to perform queries and updates on multiple threads, you'll want to use this class.

 ### See also
 
 - `<FXSDKFMDatabasePool>` - A pool of `FXSDKFMDatabase` objects.
 - `<FXSDKFMStatement>` - A wrapper for `sqlite_stmt`.
 
 ### External links
 
 - [FXSDKFMDB on GitHub](https://github.com/ccgus/FXSDKFMDB) including introductory documentation
 - [SQLite web site](http://sqlite.org/)
 - [FXSDKFMDB mailing list](http://groups.google.com/group/FXSDKFMDB)
 - [SQLite FAQ](http://www.sqlite.org/faq.html)
 
 @warning Do not instantiate a single `FXSDKFMDatabase` object and use it across multiple threads. Instead, use `<FXSDKFMDatabaseQueue>`.
...
@interface FXSDKFMDatabase : NSObject  {
...
/** Create a `FXSDKFMDatabase` object.
 
 An `FXSDKFMDatabase` is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.
 2. An empty string (`@""`).  An empty database is created at a temporary location.  This database is deleted with the `FXSDKFMDatabase` connection is closed.
 3. `nil`.  An in-memory database is created.  This database will be destroyed with the `FXSDKFMDatabase` connection is closed.

 For example, to create/open a database in your Mac OS X `tmp` folder:

    FXSDKFMDatabase *db = [FXSDKFMDatabase databaseWithPath:@"/tmp/tmp.db"];

 Or, in iOS, you might open a database in the app's `Documents` directory:

    NSString *docsPath = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
    NSString *dbPath   = [docsPath stringByAppendingPathComponent:@"test.db"];
    FXSDKFMDatabase *db     = [FXSDKFMDatabase databaseWithPath:dbPath];
 ...
 @return `FXSDKFMDatabase` object if successful; `nil` if failure.

 */

+ (instancetype)databaseWithPath:(NSString*)inPath;

/** Initialize a `FXSDKFMDatabase` object.
...
/** Execute single update statement
 ...
 @note If you want to use this from Swift, please note that you must include `FXSDKFMDatabaseVariadic.swift` in your project. Without that, you cannot use this method directly, and instead have to use methods such as `<executeUpdate:withArgumentsInArray:>`.
 */
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(FXSDKFMDBExecuteStatementsCallbackBlock)block;
...
/** Execute select statement
 ...
 Executing queries returns an `<FXSDKFMResultSet>` object if successful, and `nil` upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the `<lastErrorMessage>` and `<lastErrorMessage>` methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[FXSDKFMResultSet next]>`) from one record to the other.
 ...
 @return A `<FXSDKFMResultSet>` for the result set upon success; `nil` upon failure. If failed, you can call `<lastError>`, `<lastErrorCode>`, or `<lastErrorMessage>` for diagnostic information regarding the failure.
 
 @see FXSDKFMResultSet
 @see [`FXSDKFMResultSet next`](<[FXSDKFMResultSet next]>)
 @see [`sqlite3_bind`](http://sqlite.org/c3ref/bind_blob.html)
 
 @note If you want to use this from Swift, please note that you must include `FXSDKFMDatabaseVariadic.swift` in your project. Without that, you cannot use this method directly, and instead have to use methods such as `<executeQuery:withArgumentsInArray:>`.
 */

- (FXSDKFMResultSet *)executeQuery:(NSString*)sql, ...;
...
- (FXSDKFMResultSet *)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (FXSDKFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
- (FXSDKFMResultSet *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments;


// Documentation forthcoming.
- (FXSDKFMResultSet *)executeQuery:(NSString*)sql withVAList: (va_list)args;
...
+ (NSString*)FXSDKFMDBUserVersion;

+ (SInt32)FXSDKFMDBVersion;
...
/** Adds SQL functions or aggregates or to redefine the behavior of existing SQL functions or aggregates.
 ...
 For example:
    ...
    [queue inDatabase:^(FXSDKFMDatabase *adb) {
        ...
        int rowCount = 0;
        FXSDKFMResultSet *ars = [adb executeQuery:@"select * from ftest where StringStartsWithH(foo)"];
        while ([ars next]) {
            rowCount++;
            DLog(@"Does %@ start with 'h'?", [rs stringForColumnIndex:0]);
        }
        FXSDKFMDBQuickCheck(rowCount == 2);
    }];
...
/** Generate an `NSDateFormatter` that won't be broken by permutations of timezones or locales.
 ...
 Example:
    ...
    myDB.dateFormat = [FXSDKFMDatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
 ...
 @warning Note that `NSDateFormatter` is not thread-safe, so the formatter generated by this method should be assigned to only one FXSDKFMDB instance and should not be used for other purposes.
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using FXSDKFMDatabase::storeableDateFormat.
 ...
 @warning Note there is no direct getter for the `NSDateFormatter`, and you should not use the formatter you pass to FXSDKFMDB for other purposes, as `NSDateFormatter` is not thread-safe.
 */
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using FXSDKFMDB you will not need to interact directly with `FXSDKFMStatement`, but rather with `<FXSDKFMDatabase>` and `<FXSDKFMResultSet>` only.
...
@interface FXSDKFMStatement : NSObject {
